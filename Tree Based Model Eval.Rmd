...
# Downsample the training data due to time constaints
...
```
Initially planned to build our model with a down sampled majority class. We eventually chose against that to avoid introducing any unnecessary bias. Chose to train our models on 25000 observations as the random forest and XgBoost models took far too long to run. 
...
```
The random forest performed decently, not as well as we had hoped but it has an accuracy of ~.81 with an AUC of .71 meaning the model has moderate discriminatory power when it comes to ranking defaulters against non defaulters. The sensitivity of ~.08 tells us that our model struggles to identify defaulters, but the specificity tells us it accurately identifies non defaulters.
...
```
XgBoost Model achieved the best accuracy at 81%. Also having the largest gap from the no information rate. It again showed a strength in finding non defaulters vs identifying defaulters but the sensitivity increased when comparing it to the RF model. The AUC for this model was also 3% higher for this model meaning it does a better job ranking the two classes. 
...
```
The basic decision tree was our worst performing model, achieving comparable accuracy but showing the same pattern of high specificity and poor sensitivity, revealing limited ability to detect actual defaults. It also showed a much lower AUC meaning it has trouble ranking a defaulter vs a non defaulter at random. 


# Conclusion
In conclusion, we selected the XGBoost model as our final model as we felt it provided the best overall balance between accuracy and predictive discrimination. While all models showed similar overall accuracy, the XGBoost achieved the highest AUC (0.74), indicating stronger ability to distinguish between defaulters and non-defaulters. It also showed a notable improvement in sensitivity compared to the Random Forest and Decision Tree, meaning it identified more actual defaulters while maintaining a strong specificity.
