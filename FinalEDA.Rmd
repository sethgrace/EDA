...
#df = read.csv('M:/MSBA/ISA 591/Data/train.csv', stringsAsFactors = TRUE)
```
...
```{r}
df$mths_since_last_delinq[is.na(df$mths_since_last_delinq)] = 0
```
...
df <- df |> 
  dplyr::mutate(
    mths_since_last_delinq = dplyr::case_when(
      mths_since_last_delinq == 0 ~ "Never",
      mths_since_last_delinq >= 1 & mths_since_last_delinq <= 12 ~ "Within Last Year",
      mths_since_last_delinq > 12 ~ "1+ Years Ago",
      TRUE ~ NA_character_  # handles any missing values
    ) |> 
    factor(levels = c("Never", "Within Last Year", "1+ Years Ago"))
  )
```
...
#saveRDS(df, file = "M:/MSBA/ISA 591/EDA_cleaned.RDS")
```
